    ...
    while not stack.isEmpty():
        ...
        for node in problem.getSuccessors(currentNode[0]):
            if node[0] not in visited:
                stack.push((node[0], currentNode[1] + [node[1]]))
...
def breadthFirstSearch(problem):
    ...
    while not queue.isEmpty():
        ...
        for node in problem.getSuccessors(currentNode[0]):
            if node[0] not in visited:                
                queue.push((node[0], currentNode[1] + [node[1]]))
    util.raiseNotDefined()
    ...
    while not queue.isEmpty():
        ...
        for node in problem.getSuccessors(currentNode[0]):
            if node[0] not in visited:                
                ...
                queue.push((node[0], currentNode[1] + [node[1]]), cost)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    while not queue.isEmpty():
        ...
        for node in problem.getSuccessors(currentNode[0]):
            if node[0] not in visited:
                actions = currentNode[1] + [node[1]]
                cost = problem.getCostOfActions(actions) + heuristic(node[0], problem)
                queue.push((node[0], actions), cost)
